...
$status = Constants::SHOE_ANGLES_NUM;
...
$after_ins = $items['after_ins_3'];
$index = $items['permut_index_3'];
$stage = $items['stage_3'];
$enable = $items['enable'];
...
if(strcmp($task,Constants::SIMILARITY_INV_NUM) != 0){
...
if(strcmp($after_ins,"1") == 0){
	...
	$similarity_vectors = $myDBi->make_similarity_vectors("INV");
}
if(!empty($_POST)){
	if(isset($_POST['answers'])){
		...
		$isTrain = $arr[3];
		$time = $arr[4];
		$sql_link->query("INSERT INTO `similarity` (`phone`,`image_left`,`image_right`,`answer`,`UP_OR_INV`,`IS_TRAINING`,`time`) VALUES ('$phone','$image_left','$image_right','$ans','INV','$isTrain','$time')");
	}
	$count = $_POST['count'];
	if($count == 0) { // beginning
		$sql_link->query("UPDATE `subjects` SET `after_ins_3`='1',`stage_3`='0' WHERE `session_id`='$session_cookie'");
	}
	$count = (intval($count)+1)."";
	$stage = $_POST['stage'];
	$sql_link->query("UPDATE `subjects` SET `stage_3`='$stage' WHERE `session_id`='$session_cookie'");
	if(isset($_POST['count']) and isset($_POST['max']) && isset($_POST['stage'])){
		if($_POST['count'] == $_POST['max'] && $stage == 3){
			$sql_link->query("UPDATE `subjects` SET `permut_index_3`='0',`after_ins_3`='0',`stage_3`='0',`task_status`='$status' WHERE `session_id`='$session_cookie'");
			echo Constants::shoe_angles_page;
		}
		else if($_POST['count'] == $_POST['max']){
			$count = $_POST['count'];
			$sql_link->query("UPDATE `subjects` SET `permut_index_3`='0',`stage_3`=`stage_3`+1 WHERE `session_id`='$session_cookie'");
		}
		else{
			$sql_link->query("UPDATE `subjects` SET `permut_index_3`='$count' WHERE `session_id`='$session_cookie'");
		}
	}
	return;
}
?>
<!-- similarity inv as anchor page -->
<!DOCTYPE html>
	<html lang="he" dir="rtl">
		<head>
			...
			<title>Similarity</title>
				...
				<script type="text/javascript" src="/recognitions/dev/similarity_inv.js"></script>
							...
							<div id="section" style='margin:2.5em 2em 2em 2em;'>
								<table id="table">
									...
									<?}else{?>
										<tr>
											<td>
												<h1 class="h1_style"><strong>הוראות למטלה 3:</strong></h1>
												<?if(strcmp($gender,"בן") == 0)
												{?>
													<h4 class="h4_style">במטלה זו בדומה למטלה 2 יוצגו בפניך שתי תמונות של פנים רק הפוכות, אחת ליד השנייה ותתבקש להחליט האם התמונות זהות או שונות.</h4>
												<?}else{?>
													<h4 class="h4_style">במטלה זו בדומה למטלה 2 יוצגו בפנייך שתי תמונות של פנים רק הפוכות, אחת ליד השנייה ותתבקשי להחליט האם התמונות זהות או שונות.</h4>
												<?}?>
											</td>
										</tr>
										<tr>
										...
										<tr>
											<td>
												...
												<script type="text/javascript">
													...
													Functions.img_width = "<? echo $width; ?>";
													Functions.permut = permutation.split(",");
													console.log(Functions.permut.length);
													Functions.imgs_a = arr_a.split(",");
													Functions.imgs_b = arr_b.split(",");
													Functions.preload();
													Functions.countDown();
