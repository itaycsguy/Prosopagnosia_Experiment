var Functions = {
	...
	"uploadRest":function(){
		...
		if(Functions.gender == "בן")
		{
			str += "<tr><td><h4>במטלה זו יוצגו בפניך שתי תמונות של פנים בדומה למטלה 2, אחת ליד השנייה ותתבקש להחליט האם התמונות זהות או שונות.</h4></td></tr>";
			if(Functions.hand == "ימין")
			{
		...
		{
			str += "<tr><td><h4>במטלה זו יוצגו בפנייך שתי תמונות של פנים בדומה למטלה 2, אחת ליד השנייה ותתבקשי להחליט האם התמונות זהות או שונות.</h4></td></tr>";
			if(Functions.hand == "ימין")
			{
		...
		{
			str += "<tr><td><img alt='לחצי רווח' src='/recognitions/dev/gif_female.gif'/></td></tr>";
		}
		document.getElementById("table").innerHTML = str;
		document.addEventListener("keydown",Functions.beginTask);
	}
...
var Form = {
	"ajax":function(event){
		...
		if(Functions.img_idx < Functions.numbers_to_train && Functions.stage == 0) // 6 default
			isTraining = 1; // true
		...
		var url = "/recognitions/dev/similarity_inv.php";
	...
	"transfer":function(){
		window.location.href = "/recognitions/dev/details.php?returnTask='6'";
	}
};