...
$after_ins = $items['after_ins_4'];
$index = $items['permut_index_4'];
$stage = $items['stage_4'];
$enable = $items['enable'];
...
if(strcmp($task,Constants::GLOBAL_NUM) != 0){
...
if(strcmp($after_ins,"1") == 0){
	...
	$global_vectors = $myDBi->make_global_local_vectors('GLOBAL');
}
if(!empty($_POST)){
	if(isset($_POST['answers'])){
		...
		if(strcmp($ans,$correct) == 0)
			$correct_ans = "נכון";
		$sql_link->query("INSERT INTO `global` (`phone`,`image`,`answer`,`correct`,`correct_ans`,`IS_TRAINING`,`time`) VALUES ('$phone','$image','$ans','$correct','$correct_ans','$isTrain','$time')");
	}
	$count = $_POST['count'];
	if($count == 0) { // beginning
		$sql_link->query("UPDATE `subjects` SET `after_ins_4`='1',`task_status`='$task',`stage_4`='0' WHERE `session_id`='$session_cookie'");
	}
	$count = (intval($count)+1)."";
	$stage = $_POST['stage'];
	$sql_link->query("UPDATE `subjects` SET `stage_4`='$stage' WHERE `session_id`='$session_cookie'");
	if(isset($_POST['count']) and isset($_POST['max']) && isset($_POST['stage'])){
		if($_POST['count'] == $_POST['max'] && $stage == 29){
			$sql_link->query("UPDATE `subjects` SET `task_status`='$status',`permut_index_4`='0',`after_ins_4`='0',`stage_4`='0' WHERE `session_id`='$session_cookie'");
			echo Constants::local_page;
		}
		else if($_POST['count'] == $_POST['max']){
			$count = $_POST['count'];
			$sql_link->query("UPDATE `subjects` SET `permut_index_4`='0',`stage_4`=`stage`+1 WHERE `session_id`='$session_cookie'");
		}
		else{
			$sql_link->query("UPDATE `subjects` SET `permut_index_4`='$count' WHERE `session_id`='$session_cookie'");
		}
	}
	return;
}
...
<!DOCTYPE html>
	<html lang="he" dir="rtl">
		<head>
			...
			<meta name="description" content="show a shape which is circle or aquere that built up of little circle or aquere, need to decide which is the outter shape"/>
			...
			<title>Global</title>
				...
				<script type="text/javascript" src="/recognitions/dev/global.js"></script>
								...
								<table id="table">
									<?if($need_new_content == true){?>
										<script type="text/javascript">
											...
											var arr_a = "<? echo $global_vectors[1]; ?>";
											var arr_b = "<? echo $global_vectors[3]; ?>"; //outter
											...
											console.log("Functions.permut=" + permutation);
									...
									<?}else{?>
										<tr>
											<td>
												<h1 class="h1_style"><strong>הוראות למטלה 6:</strong></h1>
												<h4 class="h4_style">במטלה הבאה תופיע צורה גדולה שמורכבת מצורות קטנות, </h4>
												<h4 class="h4_style">עליך להחליט מהי הצורה של הצורה <strong><label style='font-size:18px'>הגדולה</label></strong>.</h4>
											</td>
										</tr>
										<tr>
											<td>
										...
										<tr>
											<td>
												<? 
												$global_vectors = $myDBi->make_global_local_vectors("GLOBAL");
												$permut = $myDBi->make_permutation($global_vectors[0]);
												...
												<script type="text/javascript">
													...
													Functions.imgs_a = arr_a.split(",");
													Functions.imgs_b = arr_b.split(",");
													//console.log(Functions.imgs_a);
													//console.log(Functions.imgs_b);
													Functions.preload();
													Functions.countDown();
													Functions.putGIF();
												</script>
