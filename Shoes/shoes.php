...
$status = Constants::FACES_ANGLES_NUM;
...
$after_ins = $items['after_ins_shoes'];
$curr_type = $items['shoes_type'];
$enable = $items['enable'];
$need_new_content = false;
$shoes_vectors = $myDBi->make_shoes_vectors();
$index_1 = 0;
$index_2 = 0;
$index_3 = 0;
...
if(strcmp($task,Constants::SHOE_ANGLES_NUM) != 0) {
...
if(strcmp($after_ins,"1") == 0) {
	...
	$shoes1 = $sql_link->query("SELECT MAX(next_index) AS max FROM `shoes_1` WHERE `phone`='$phone'");
	$shoes1_vec = $shoes1->fetch_array(MYSQLI_ASSOC);
	$shoes2 = $sql_link->query("SELECT MAX(next_index) AS max FROM `shoes_2` WHERE `phone`='$phone'");
	$shoes2_vec = $shoes2->fetch_array(MYSQLI_ASSOC);
	$shoes3 = $sql_link->query("SELECT MAX(next_index) AS max FROM `shoes_3` WHERE `phone`='$phone'");
	...
	$sql_link->query("UPDATE `subjects` SET `after_ins_shoes`='1',`shoes_type`='1' WHERE `session_id`='$session_cookie'");
}
if(!empty($_POST)) {
	...
	if($type == 1) {
		...
		$clock = $arr[7];
		$next_idx = $arr[8];
		$sql_link->query("INSERT INTO `shoes_1` (`phone`,`imgLeft`,`imgMiddle`,`imgRight`,`answer`,`correct`,`IS_TRAINING`,`time`,`next_index`) VALUES ('$phone','$imgL','$imgM','$imgR','$answer','$correct','$is_training','$clock','$next_idx')");
		$sql_link->query("UPDATE `subjects` SET `shoes_type`='1' WHERE `session_id`='$session_cookie'");
	} else if($type == 2) {
		$image = $arr[2];
		$correct = $arr[3];
		$is_training = $arr[4];
		$clock = $arr[5];
		$next_idx = $arr[6];
		$sql_link->query("INSERT INTO `shoes_2` (`phone`,`img`,`answer`,`correct`,`IS_TRAINING`,`time`,`next_index`) VALUES ('$phone','$image','$answer','$correct','$is_training','$clock','$next_idx')");
		$sql_link->query("UPDATE `subjects` SET `shoes_type`='2' WHERE `session_id`='$session_cookie'");
	} else if($type == 3) {
		$image = $arr[2];
		$correct = $arr[3];
		$is_training = $arr[4];
		$clock = $arr[5];
		$next_idx = $arr[6];
		$sql_link->query("INSERT INTO `shoes_3` (`phone`,`img`,`answer`,`correct`,`IS_TRAINING`,`time`,`next_index`) VALUES ('$phone','$image','$answer','$correct','$is_training','$clock','$next_idx')");
		$sql_link->query("UPDATE `subjects` SET `shoes_type`='3' WHERE `session_id`='$session_cookie'");
	}
	...
	if($type == 1 && $v_next <= 7) {
		...
		$sql_link->query("UPDATE `subjects` SET `task_status`='$status',`after_ins_shoes`='0',`shoes_type`='1' WHERE `session_id`='$session_cookie'");
		echo Constants::faces_angles_page;
		return;
	}
	return;
}
?>
<!-- shoes up as anchor page -->
<!DOCTYPE html>
	<html lang="he" dir="rtl">
		<head>
			...
			<link href="/recognitions/dev/shoes.css" rel="stylesheet"/>
			<title>Shoes</title>
				...
				<script type="text/javascript" src="/recognitions/dev/shoes.js"></script>
								...
								<table id="table">
									<?if($need_new_content == true){?>
										<script type="text/javascript">
											...
											var type1a = "<? echo $shoes_vectors[0][1]; ?>";
											var type1b = "<? echo $shoes_vectors[0][3]; ?>";
											var type1c = "<? echo $shoes_vectors[0][5]; ?>";
											var type2a = "<? echo $shoes_vectors[1][1]; ?>";
											var type2b = "<? echo $shoes_vectors[1][3]; ?>";
											var type3a = "<? echo $shoes_vectors[2][1]; ?>";
											var type3b = "<? echo $shoes_vectors[2][3]; ?>";
											...
											console.log("img_idx_1 => " + Functions.img_idx_1);
											Functions.preload();
											if(Functions.type == 1){
									...
									<?}else{?>
										<tr>
											<td>
												<h1 class="h1_style"><strong>הוראות למטלה 4:</strong></h1>
												<h4 class="h4_style">בשלב הראשון של משימה זו, עלייך להתבונן היטב בקבוצה של תמונות נעליים.</h4>
												<h4 class="h4_style">לאחריהן, יופיע מקבץ של תמונות נעליים שונות.</h4>
												<?if(strcmp($gender,"בן") == 0){?>
														<h4 class="h4_style">אתה מתבקש לציין מי מהנעליים הללו הופיעו קודם.</h4>
												<?}else if(strcmp($gender,"בת") == 0){?>
														<h4 class="h4_style">את מתבקשת לציין מי מהנעליים הללו הופיעו קודם.</h4>
												<?}?>
											</td>
										</tr>
										<tr>
